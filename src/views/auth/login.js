...
import { Box, VStack, HStack, Center, FormControl, Link, WarningOutlineIcon, Input, Button, Heading, Image, Text, safeArea } from "native-base";
                ...
                <Box safeArea p="2" py="8" w="full" px={5}>
                    ...
                    <VStack space={3} mt="5">
                        <FormControl isInvalid>
                            ...
                            {isFieldInError('username') && <FormControl.ErrorMessage leftIcon={<WarningOutlineIcon size="xs" />}>
                                Email cannot be empty.
                            </FormControl.ErrorMessage>}
                        </FormControl>
                        {((!isBiometricEnabled && !isEnabledBioAuth) || failureCount > 2 ) && <FormControl isInvalid>
                            <FormControl.Label _text={{
                                color: "#ffffff",
                                fontWeight: "medium",
                                fontSize: "sm"
                        ...
                        <HStack space={isBiometricEnabled ? 1 : 0}>
                            <Button
                                isDisabled={isBiometricEnabled && isEnabledBioAuth && !password?.length}
                                isLoading={isLoading}
                                onPress={() => { authenticate() }}
                                mt={5} variant={'solid'} w={"full"} style={{ ...Shared.Button.primary, flex: 1 }} >
                                Log In
                            </Button>
                            {isBiometricEnabled && failureCount <= 2 && <Button
                                isLoading={isLoading}
                                onPress={() => { 
                                    setFailureCount(0)
                                    configureBiometricAuth()
                                 }}
                                mt={5} variant={'ghost'} w={"full"} style={{ width: 50, color: '#ffffff' }} >
