...
const HomeScreen = ({ navigation }) => {
  ...
  const { fetchData: getAccountTypes } = useAuthentication(
    "getaccounttypes",
    "get",
    navigation
  );
  const { fetchData: fetchInvestments } = useAuthentication(
    "getinvestments",
    "get",
    navigation
  );
  const { fetchData: fetchAccountBalance } = useAuthentication(
    "getaccountbalance",
    "get",
    navigation,
    false
                ...
                >
                                  ...
                                  (accountData, indx) => (
                                    <Select.Item
                                      ...
                                      label={`${accountData.label} - ${
                                        accountData.userAccount?.account_info
                  ...
                  <Box mt={2} p={3} h={100}>
                    <Center>
                      {!acctLoading && (
                        <Box>
                          <HStack>
                            ...
                            <IconButton
                              onPress={() =>
                                setDisplayAccountBalance(!displayAccountBalance)
                              }
                              ml={5}
                              mt={2}
                              icon={
                          ...
                          <HStack>
                            ...
                            {displayAccountBalance && (
                              <Currency
                                ...
                                color={Theme.Colors.primaryText}
                                fontWeight={"bold"}
                                value={accountBalanceInfo?.ledgerBalance ?? 0.0}
                              />
                            )}
                            {!displayAccountBalance && (
                              <Text
                      ...
                      <Box>
                        {acctLoading && (
                          <Text fontSize={16} color={"amber.100"}>
                            Refreshing Balance...
                          </Text>
                        )}
                      </Box>
          ...
          {(!investments || investments.length == 0) && (
            <Box mt={3} px={3.5} w={Shared.DeviceDimensions.WIDTH}>
              <Card
                ...
                subtitle={`Create investment account to enjoy the best interest rates`}
                actionBtn={
                  <Box>
                    <Button
                      onPress={() =>
        ...
        <Actionsheet.Content style={{ backgroundColor: "#f9f9f9" }}>
          <Box mb={7} w="100%" h={100} px={4} justifyContent="center">
            ...
            <Box mt={3}>
              ...
              >
                {!hasAccount
                  ? "You don't seem to have any existing account. Create a new bank account or link all your accounts if you already have accounts with Imperial Homes Mortgage Bank"
                  : ""}
              </Text>
            </Box>
          </Box>
            ...
            <Button
              ...
              onPress={() => {
                ...
                navigation.navigate("CreateAccountScreen", {
                  accountType: accountToOpen.name,
                  accountTypeLabel: accountToOpen.label,
                  authData,
                });
              }}
            >
          ...
          <Box mt={5} mb={5}>
            <Text fontWeight={"extrabold"} color={"#000000"}>
              OR LINK YOUR
            </Text>
          </Box>
          <Flex justify="space-between" mt={5} mb={10} direction="row">
            {Object.values(accountTypes)
              .filter((t) => !userAccountMap.includes(t.name))
              .map((accountType) => {
                return (
                  <Button
                    ...
                    colorScheme={"orange"}
                    variant={"outline"}
                    style={Shared.Button.primary_outline}
                    onPress={() => {
                      onClose();
