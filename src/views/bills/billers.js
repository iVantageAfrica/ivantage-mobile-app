...
const BillersScreen = ({ navigation, route }) => {
    ...
    const renderItem = ({ item, index }) => {
        ...
        key={item.id} onPress={() => navigation.navigate('bills.billers.packages', item)} title={item.name}  icon={<Icon size={5} color={Theme.Colors.tertiaryText} as={FontAwesome} name="chevron-right" />} />
    }
